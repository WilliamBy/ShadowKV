...
class Evaluator:
    ...
    def test(self, llm: LLM, dataset: Dataset, output_path: str, setting: str = 'baseline'):
        ...
        if self.dist_config.master_process:
            print(colored(
                f"[Test] {llm.model_name} on {dataset.dataset_name}, results saved to {output_path}", 'green'))
        ...
        for i in range(dataset.num_samples // bsz):
            prompt = torch.cat([dataset.tokenized_prompts[i*bsz+j]
                               for j in range(bsz)], dim=0)
            if 'persona' in dataset.dataset_name:
                assert bsz == 1
                llm.generate(prompt.to(llm.device), gen_len=0, verbose=False,
                             top_p=0.9, temperature=0.0)  # prefill ctx
                queries = dataset.queries[i]
                gts_list = dataset.gt[i]
                rets_list = []
                for query, gts in zip(queries, gts_list):
                    rets = llm.generate(llm.encode(
                        query, template="chat"), cont=True, gen_len=dataset.gen_len, top_p=0.9, temperature=0.0)
    ...
    def summarize(self):
        ...
        if self.dist_config.is_distributed:
            ...
            dist.gather_object(
                df, output if self.dist_config.master_process else None, dst=0)
            dist.barrier()
            if self.dist_config.master_process:
                df = pd.concat(output)
                # Define the columns you want to calculate the mean for (excluding 'samples')
                setting_columns = [col for col in df.columns if col not in [
        ...
        if self.dist_config.master_process:
            ...
            mean_row = pd.DataFrame(
                {col: [mean_values[col] if col in mean_values else 'mean'] for col in df.columns})
