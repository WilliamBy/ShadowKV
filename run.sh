...
# $PY test/eval_acc.py --datalen $DATALEN --method tova --dataset_name $DATASETS --model_name "$MODEL_NAME" --sparse_budget $SPARSE --chunk_size 8 | tee llama-tova-lbc.log || exit 1

# Run with TOVA attention
$PY test/eval_acc.py --datalen $DATALEN --method tova --dataset_name $DATASETS --model_name "$MODEL_NAME" --sparse_budget $SPARSE | tee llama-tova-lbc.log || exit 1
# $PY test/eval_acc.py --datalen $DATALEN --method tova --dataset_name $DATASETS --model_name "$MODEL_NAME" --sparse_budget $SPARSE | tee llama-tova-ruler.log || exit 1

# Run with Optimized attention
$PY test/eval_acc.py --datalen $DATALEN --method optimized --dataset_name $DATASETS --model_name "$MODEL_NAME" --sparse_budget $SPARSE --chunk_size 8 | tee llama-opt-lbc.log || exit 1
